...
public class ParallelCam : MonoBehaviour
{
    //Thread
    private Thread myThread;
    public List<Tuple<List<Tuple<Tuple<int, int>, float>>, List<int>>> latestval = new List<Tuple<List<Tuple<Tuple<int, int>, float>>, List<int>>>();
    ...
    {
        if (this.transform.name == "Drone_red (41)")
            Debug.Log("Here");
    ...
    /*void Update()
    {
        ...
        if (startit)
        {
            ...
            for (int i = 0; i < latestval[camera_num].Item1.Count; i++)
            {
                int pix_x = latestval[camera_num].Item1[i].Item1.Item2;
                int pix_y = latestval[camera_num].Item1[i].Item1.Item1;
                float z_depth = latestval[camera_num].Item1[i].Item2;
                float disp_x = 0;
                float disp_y = 0;
                ...
                if (pix_y > 128)
                {
                    ...
                    disp_y = (Mathf.Sin(angle * Mathf.PI / 180) * z_depth);
                }
                else if (pix_y < 130)
                {
                ...
                if (pix_x <= 128)
                {
                    ...
                    disp_x = -(Mathf.Sin((halfAngle[camera_num] - angle) * Mathf.PI / 180) * z_depth);
                    z_depth = (Mathf.Cos((halfAngle[camera_num] - angle) * Mathf.PI / 180) * z_depth);
                }
                else if (pix_x > 128)
                {
                    if (pix_x < 230)
                    {
                        ...
                        disp_x = -(Mathf.Cos((90 - halfAngle[camera_num] - angle) * Mathf.PI / 180) * z_depth);
                        z_depth = (Mathf.Sin((90 - halfAngle[camera_num] - angle) * Mathf.PI / 180) * z_depth);

                    }
                    else if (pix_x > 230)
                    {
                        float angle = (pix_x - 128) * halfAngle[camera_num] / 128f;
                        disp_x = -(Mathf.Cos((angle + halfAngle[camera_num] - 90) * Mathf.PI / 180) * z_depth);
                        z_depth = -(Mathf.Sin((angle + halfAngle[camera_num] - 90) * Mathf.PI / 180) * z_depth);
                    }
                    else
                    {
                        disp_x = -z_depth;
